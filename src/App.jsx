...
padding: 0 24px; height: 46px; font-size: 14px; font-weight: 700;
cursor: pointer; transition: all .15s; letter-spacing: .3px;
display: flex; align-items: center; justify-content: center; gap: 8px;
}
.btn-primary:hover { background: #1D4ED8; transform: translateY(-1px); box-shadow: 0 8px 20px ${T.accent}40; }
.btn-primary:active { transform: translateY(0); }

.btn-ghost {
background: transparent; color: ${T.textSub}; border: 1.5px solid ${T.border};
border-radius: 10px; padding: 0 20px; height: 40px; font-size: 13px;
...
.card {
background: ${T.white}; border: 1.5px solid ${T.border}; border-radius: 16px;
...
border-radius: 10px; padding: 0 14px; height: 46px; font-size: 14px;
color: ${T.text}; outline: none; transition: border-color .15s, box-shadow .15s;
}
.input-field:focus { border-color: ${T.accent}; box-shadow: 0 0 0 3px ${T.accent}18; background: ${T.white}; }
...
color: rgba(255,255,255,.55); font-size: 13.5px; font-weight: 500; width: 100%;
font-family: ‘Barlow’, sans-serif; text-align: left;
}
.sidebar-link:hover { background: ${T.sidebarHover}; color: rgba(255,255,255,.85); }
.sidebar-link.active { background: ${T.sidebarActive}; color: #fff; font-weight: 700; box-shadow: 0 4px 14px ${T.accent}50; }
...
font-size: 11px; font-weight: 700; letter-spacing: .4px; font-family: ‘Barlow’, sans-serif;
}

.notification-dot { width: 7px; height: 7px; background: ${T.red}; border-radius: 50%; position: absolute; top: 2px; right: 2px; border: 2px solid ${T.sidebar}; }
...
background-image: linear-gradient(${T.accent}12 1px, transparent 1px), linear-gradient(90deg, ${T.accent}12 1px, transparent 1px);
...
{ id: “U4”, nome: “João Silva”,    login: “joao”,   senha: “1234”, role: “conferente”, avatar: “JS” },
];
...
{ cod:“80001234”, nome:“GUARANÁ ANTARCTICA 2L”,       ean:“7891991010856”, local:“B-01-004”, lote:“41”, val:“02/10/25”, est:216, un:“UN” },
...
const PEDIDOS = [
{ id:“PED-8821”, cliente:“Mercado São João”,     valor:4820.50, itens:14, status:“separando”,  data:“06/05/25”, vendedor:“Marcos” },
...
{ id:“PED-8818”, cliente:“Padaria Pão de Mel”,   valor:890.30,  itens:5,  status:“entregue”,   data:“05/05/25”, vendedor:“Luana” },
{ id:“PED-8817”, cliente:“Empório da Família”,   valor:2340.00, itens:11, status:“entregue”,   data:“04/05/25”, vendedor:“Tiago” },
...
const NOTAS = [
{ nf:“NF-004521”, cliente:“Mercado São João”,   valor:4820.50, emissao:“06/05/25”, venc:“20/05/25”, status:“aberta” },
{ nf:“NF-004520”, cliente:“Atacarejo Boa Vida”, valor:9670.80, emissao:“05/05/25”, venc:“19/05/25”, status:“aberta” },
{ nf:“NF-004519”, cliente:“Empório da Família”, valor:2340.00, emissao:“04/05/25”, venc:“18/05/25”, status:“paga” },
{ nf:“NF-004518”, cliente:“Padaria Pão de Mel”, valor:890.30,  emissao:“03/05/25”, venc:“17/05/25”, status:“paga” },
{ nf:“NF-004515”, cliente:“Mini Box Central”,   valor:1100.00, emissao:“28/04/25”, venc:“12/05/25”, status:“vencida” },
];

const ENTREGAS = [
{ idx:1, cliente:“Mercado São João”,    end:“Rua XV de Novembro, 340 – Centro”, vol:12, peso:28.4, status:“entregue”,   hora:“09:15” },
{ idx:2, cliente:“Mini Box Central”,   end:“Av. Brasil, 1200 – Jd. América”,   vol:7,  peso:14.2, status:“em_andamento”,hora:“10:40” },
{ idx:3, cliente:“Atacarejo Boa Vida”, end:“Rua Campos Salles, 85 – Bela Vista”,vol:23, peso:41.0, status:“aguardando”, hora:”–” },
{ idx:4, cliente:“Padaria Pão de Mel”, end:“Rua da Paz, 22 – São Lucas”,       vol:5,  peso:8.1,  status:“aguardando”, hora:”–” },
];
...
const S_LABEL = { em_andamento:“Em Andamento”, em_rota:“Em Rota”, aguardando:“Aguardando”, concluido:“Concluído”, separando:“Separando”, conferencia:“Conferência”, entregue:“Entregue”, cancelado:“Cancelado”, aberta:“Em Aberto”, paga:“Paga”, vencida:“Vencida” };
...
const c = sc(status); const label = children || sl(status);
return <span className=“badge” style={{ background: c + “18”, color: c }}>
...
return <div>
{label && <div style={{ display:“flex”, justifyContent:“space-between”, fontSize:12, color:T.textSub, marginBottom:6 }}>
  ...
  <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:420, padding:"0 20px" }}>
    ...
    <div className="fade-up" style={{ textAlign:"center", marginBottom:40 }}>
      <div style={{ display:"inline-flex", alignItems:"center", gap:10, marginBottom:10 }}>
        ...
        <div>
          <div className="condensed" style={{ fontSize:26, fontWeight:800, color:"#fff", lineHeight:1, letterSpacing:-.5 }}>DistribFácil</div>
    ...
    <div className="fade-up-1 scale-in" style={{ background:"rgba(255,255,255,.06)", backdropFilter:"blur(20px)", border:"1.5px solid rgba(255,255,255,.1)", borderRadius:20, padding:32 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div>
          <label style={{ fontSize:12, fontWeight:600, color:"rgba(255,255,255,.5)", display:"block", marginBottom:6, letterSpacing:.5 }}>USUÁRIO</label>
        ...
        <button className="btn-primary" onClick={handleLogin} disabled={loading} style={{ height:50, fontSize:15, marginTop:4, opacity:loading?.85:1 }}>
    ...
    <div className="fade-up-2" style={{ marginTop:24 }}>
      <div style={{ fontSize:11, color:"rgba(255,255,255,.3)", textAlign:"center", marginBottom:12, letterSpacing:1 }}>ACESSO RÁPIDO (DEMO)</div>
...
<div className=“fade-up-1” style={{ fontSize:13, color:T.textMuted }}>Visão geral do dia — 06/05/2025</div>
</div>
...
<KPI label="Clientes Atendidos" value="35" sub="+8% vs semana ant." color={T.purple} icon="🏪" />
</div>
...
```
  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:24 }}>
    ...
    <Section title="Últimos Pedidos" delay="-3">
      <div className="card" style={{ padding:"8px 0" }}>
        {PEDIDOS.slice(0,5).map(p => (
          <div key={p.id} className="table-row" style={{ gridTemplateColumns:"1fr auto", marginBottom:0 }}>
            <div>
  ...
  <Section title="Notas Fiscais Recentes" delay="-4">
    <div className="card" style={{ padding:0, overflow:"hidden" }}>
      <div style={{ display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr 1fr 1fr", padding:"10px 16px", background:T.bg, fontSize:11, fontWeight:700, color:T.textMuted, letterSpacing:.5, textTransform:"uppercase" }}>
        <span>NF / Cliente</span><span>Emissão</span><span>Vencimento</span><span style={{textAlign:"right"}}>Valor</span><span style={{textAlign:"right"}}>Status</span>
      </div>
      {NOTAS.map((n,i) => (
...
```
  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:14, marginBottom:24 }}>
    ...
    <KPI label="Vencido" value={fmtR(vencidas)} color={T.red} icon="⚠" sub="Cobrança necessária" />
  </div>
  ...
  <Section title="Notas Fiscais" delay="-3">
    <div className="card" style={{ padding:0, overflow:"hidden" }}>
      <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr", padding:"10px 20px", background:T.bg, fontSize:11, fontWeight:700, color:T.textMuted, letterSpacing:.4, textTransform:"uppercase" }}>
        <span>Cliente</span><span>NF</span><span>Emissão</span><span style={{textAlign:"right"}}>Valor</span><span style={{textAlign:"right"}}>Situação</span>
      </div>
      {NOTAS.map((n,i) => (
        ...
        <div key={r.id} className={`card card-clickable fade-up`} style={{ animationDelay:`${i*.05}s`, borderLeft:`4px solid ${sc(r.status)}` }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr auto", gap:16, marginBottom:16 }}>
            <div>
              ...
              <div style={{ fontSize:12, color:T.textMuted, marginTop:2 }}>🚚 {r.motorista} · Saída: {r.saida} · Prev: {r.prev}</div>
            </div>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:16 }}>
            {[["Clientes",r.cliente_count,T.accent],["Itens",r.itens,T.purple],["Peso",`${r.peso}kg`,T.amber],["Volume",`${r.vol}m³`,T.cyan]].map(([l,v,c])=>(
...
const Atividade = () => (
  ...
  <div>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
      ...
      <KPI label="Progresso" value="32/48" sub="66% concluído" color={T.green} icon="📦" />
    </div>
    <div className="card fade-up-2" style={{ marginBottom:14 }}>
      <div style={{ fontSize:12, fontWeight:700, color:T.textMuted, marginBottom:14, letterSpacing:.5, textTransform:"uppercase" }}>Distribuição por Rua</div>
    ...
    <div className="card fade-up-3" style={{ marginBottom:14 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14, marginTop:16 }}>
        {[["Peso","103.5 kg",T.accent],["Volume","0.16 m³",T.purple],["Clientes","12",T.green]].map(([l,v,c])=>(
          <div key={l} style={{ textAlign:"center" }}>
...
return (
<div>
<div className=“fade-up condensed” style={{ fontSize:26, fontWeight:800, marginBottom:6 }}>Separação</div>
<div className=“fade-up-1” style={{ fontSize:13, color:T.textMuted, marginBottom:20 }}>Roteiro 225151 · BAU_PRES PRUDENTE</div>

```
  {/* Product card */}
  <div className={`card fade-up-1`} style={{ marginBottom:14, borderLeft:`4px solid ${T.accent}`, transition:"border-color .3s", borderLeftColor:flash?T.green:T.accent }}>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
      <div style={{ background:T.bg, borderRadius:8, padding:"10px 12px" }}>
        ...
        <div className="mono" style={{ fontSize:14, fontWeight:600 }}>Nº {prod.lote}</div>
      </div>
      <div style={{ background:T.bg, borderRadius:8, padding:"10px 12px" }}>
  ...
  <div className="card fade-up-2" style={{ marginBottom:14 }}>
    <div style={{ marginBottom:10 }}>
      ...
      <div style={{ fontSize:11, color:T.textMuted, marginBottom:6 }}>EAN / CÓD. BARRAS</div>
    ...
    <input className="input-field" value={input} onChange={e=>setInput(e.target.value)}
      ...
      placeholder="Bipe ou digite o código..." autoFocus />
...
setLoading(false);
found ? setRes(found) : setMiss(true);
}, 700);
};
...
<div className=“fade-up-1” style={{ fontSize:13, color:T.textMuted, marginBottom:20 }}>Bipe ou pesquise por EAN, código, nome ou local</div>
...
```
  <div className="card fade-up-1" style={{ marginBottom:16 }}>
    ...
    <div style={{ display:"flex", gap:10 }}>
      <input className="input-field" value={q} onChange={e=>setQ(e.target.value)}
        onKeyDown={e=>e.key==="Enter"&&search()} placeholder="EAN, código, nome, local..." autoFocus />
  ...
  {miss && <div className="card scale-in" style={{ borderColor:T.red, borderLeftWidth:4, marginBottom:16, textAlign:"center", padding:24 }}>
    <div style={{ fontSize:32, marginBottom:8 }}>🔍</div>
    <div style={{ fontWeight:700, color:T.red }}>Produto não encontrado</div>
    <div style={{ fontSize:13, color:T.textMuted, marginTop:4 }}>Verifique o código EAN ou nome informado</div>
  </div>}
  ...
  {res && (
    <div className="card scale-in" style={{ borderColor:T.green, borderLeftWidth:4, marginBottom:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        {[["Local",res.local,T.accent],["Estoque",`${res.est} ${res.un}`,T.green],["Lote Nº",res.lote,T.text],["Validade",res.val,T.text]].map(([k,v,c])=>(
  ...
  <div style={{ fontSize:12, fontWeight:700, color:T.textMuted, marginBottom:10, letterSpacing:.4, textTransform:"uppercase" }}>Acesso Rápido</div>
...
const Historico = () => (
  ...
  <div>
    <div className="fade-up condensed" style={{ fontSize:26, fontWeight:800, marginBottom:6 }}>Histórico</div>
...
const Produtos = () => {
const [q, setQ] = useState(””);
const filtered = PRODUCTS.filter(p => !q || p.nome.toLowerCase().includes(q.toLowerCase()) || p.ean.includes(q) || p.cod.includes(q) || p.local.toLowerCase().includes(q.toLowerCase()));
return (
<div>
...
<input className=“input-field fade-up-1” value={q} onChange={e=>setQ(e.target.value)} placeholder=“Filtrar por nome, EAN, código ou local…” style={{ marginBottom:16 }} />
...
return (
<div>
<div className=“fade-up condensed” style={{ fontSize:26, fontWeight:800, marginBottom:6 }}>Conferência</div>
<div className=“fade-up-1” style={{ fontSize:13, color:T.textMuted, marginBottom:16 }}>Pedido PED-8821 · Mercado São João</div>
<div className=“card fade-up-1” style={{ marginBottom:16 }}>
<Progress value={done} max={items.length} color={done===items.length?T.green:T.accent} label={done===items.length?“✅ Conferência completa!”:`${items.length-done} item(s) pendentes`} />
</div>
{items.map((p,i) => (
<div key={p.cod} onClick={()=>setChecked(c=>({…c,[p.cod]:!c[p.cod]}))}
className=“card card-clickable fade-up” style={{ animationDelay:`${i*.04}s`, marginBottom:8, display:“flex”, alignItems:“center”, gap:12,
background:checked[p.cod]?T.greenLight:T.white, borderColor:checked[p.cod]?T.green:T.border }}>
<div style={{ width:26, height:26, borderRadius:8, background:checked[p.cod]?T.green:T.bg, border:`2px solid ${checked[p.cod]?T.green:T.border}`, display:“flex”, alignItems:“center”, justifyContent:“center”, flexShrink:0, transition:“all .15s” }}>
...
{ENTREGAS.map((e,i) => (
<div key={e.idx} className=“card card-clickable fade-up” style={{ animationDelay:`${i*.05}s`, borderLeft:`4px solid ${sc(e.status)}`, background:sel===e.idx?T.bg:T.white }}
...
const RoteiroMot = () => (
  ...
  <div>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
      <KPI label="Saída" value="08:30" color={T.accent} icon="🕐" />
      <KPI label="Prev. Retorno" value="15:00" color={T.purple} icon="🏁" />
    </div>
    <div className="card fade-up-2">
      <div style={{ fontSize:12, fontWeight:700, color:T.textMuted, marginBottom:14, letterSpacing:.4, textTransform:"uppercase" }}>Informações do Roteiro</div>
      {[["Roteiro","225151"],["Rota","PRES PRUDENTE – ZONA SUL"],["Veículo","VAN-ABC1234"],["Motorista","Pedro Lima"]].map(([k,v])=>(
    ...
    <div className="card fade-up-3" style={{ marginTop:14 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14, marginTop:16 }}>
        {[["Clientes",4,T.accent],["Peso","92 kg",T.amber],["Volume","0.16 m³",T.purple]].map(([l,v,c])=>(
          <div key={l} style={{ textAlign:"center" }}>
            <div className="condensed" style={{ fontSize:22, fontWeight:800, color:c }}>{v}</div>
            <div style={{ fontSize:11, color:T.textMuted }}>{l}</div>
          </div>
...
<div>
<div className=“condensed” style={{ fontSize:20, fontWeight:800, color:”#fff”, letterSpacing:-.3 }}>DistribFácil</div>
  ...
  <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
    ...
    <div style={{ background:T.white, borderBottom:`1.5px solid ${T.border}`, padding:"0 24px", height:60, display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0, position:"sticky", top:0, zIndex:10 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:16 }}>
        ...
        <span className="mono" style={{ fontSize:12, color:T.textMuted }}>12:36</span>
        <div style={{ width:1, height:20, background:T.border }} />
    ...
    {/* Bottom Nav (mobile) */}
    <div className="bottom-nav" style={{ position:"fixed", bottom:0, left:0, right:0, background:T.white, borderTop:`1.5px solid ${T.border}`, padding:"6px 0 env(safe-area-inset-bottom)", zIndex:50 }}>
      {nav.map(n => (
        <button key={n.id} onClick={()=>setScreen(n.id)} style={{
          flex:1, background:"none", border:"none", cursor:"pointer", padding:"8px 4px", display:"flex", flexDirection:"column", alignItems:"center", gap:3,
